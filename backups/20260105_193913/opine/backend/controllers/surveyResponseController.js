...
const getSurveyResponseById = async (req, res) => {
  try {
    ...
    const surveyResponse = await SurveyResponse.findById(responseId)
      ...
      .select('survey interviewer status responses location metadata interviewMode selectedAC audioRecording createdAt updatedAt startedAt completedAt totalTimeSpent completionPercentage responseId call_id');
...
const getInterviewerStats = async (req, res) => {
  try {
    const interviewerId = req.user.id;
    const interviewerObjectId = mongoose.Types.ObjectId.isValid(interviewerId) 
      ...
      },
      {
        $group: {
          ...
          totalCompleted: {
            $sum: {
              $cond: [
                { $in: ['$status', ['Approved', 'Rejected', 'Pending_Approval']] },
                1,
                0
              ]
            }
    ...
    console.log('📊 getInterviewerStats - Aggregation results:', JSON.stringify(result, null, 2));
...
const getQualityAgentStats = async (req, res) => {
  try {
    ...
    console.log('📊 getQualityAgentStats - Aggregation results:', JSON.stringify(result, null, 2));
