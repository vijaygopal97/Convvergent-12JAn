 ...
 * Date Range: December 30-31, 2024
 * 
 * Requirements:
 * 1. Find PM: dulal.roy@convergent.com
 * 2. Get all assigned interviewers from assignedTeamMembers
 * 3. Get all AUTO-REJECTED interviews (not manual rejections) from those interviewers between Dec 30-31, 2024
...
async function generateReport() {
  try {
    ...
    // Step 3: Get all AUTO-REJECTED responses between Dec 30-31, 2024
    console.log('📋 Step 2: Fetching AUTO-REJECTED responses between December 30-31, 2024...');
    ...
    // Get all responses from this PM's interviewers before Dec 30 to check for duplicates
    const earlierResponses = await SurveyResponse.find({
      interviewer: { $in: interviewerIds },
    ...
    const earlierContentHashes = new Set(earlierResponses.map(r => r.contentHash));
    ...
    for (const response of autoRejectedResponses) {
      ...
      // Check if this contentHash exists in earlier responses OR in current batch
      if (earlierContentHashes.has(contentHash) || seenContentHashes.has(contentHash)) {
        ...
        duplicateResponses.push({
          ...
          isEarlierDuplicate: earlierContentHashes.has(contentHash)
        });
      } else {
    ...
    const reportData = {
      metadata: {
        ...
        reportDateRange: {
          ...
          description: 'December 30-31, 2024'
        },
    ...
    const filename = `Auto_Rejected_Interviews_Report_Dec30_31_${pm.firstName}_${pm.lastName}_${new Date().toISOString().split('T')[0]}.json`;
    ...
    console.log(`Date Range: December 30-31, 2024`);
