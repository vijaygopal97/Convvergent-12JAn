...
// Loading screen removed - backend optimized with aggregations for instant load

const CallerPerformanceV2Page = () => {
  ...
  // CRITICAL: Refs for memory leak prevention (like top tech companies)
  // isMountedRef: Track if component is mounted to prevent state updates on unmounted components
  const isMountedRef = useRef(true);
  // abortControllerRef: Manage AbortController instances for API request cancellation
  ...
  // OPTIMIZED: Fetch CATI stats from backend with localStorage caching
  // Top tech companies use client-side caching for instant display
  const fetchCatiStats = async (currentFilters = catiFilters, isFilterChange = false) => {
    try {
      ...
      const cacheKey = `cati-stats-${surveyId}-${JSON.stringify(currentFilters)}`;
      ...
      if (cachedData && cachedTimestamp && !isFilterChange) {
        const age = Date.now() - parseInt(cachedTimestamp);
        if (age < CACHE_DURATION) {
          try {
            const stats = JSON.parse(cachedData);
            setCatiStats(stats);
            console.log('✅ Using cached CATI stats (age:', Math.round(age / 1000), 's)');
          } catch (e) {
      ...
      if (catiStatsResponse && catiStatsResponse.success) {
        ...
        localStorage.setItem(cacheKey, JSON.stringify(catiStatsResponse.data));
  ...
  // OPTIMIZED: Fetch survey and filter options efficiently (no loading 10K responses)
  // Top tech companies use lightweight endpoints for filter options
  const fetchSurveyData = async () => {
    try {
      ...
      // OPTIMIZED: Use getSurveyResponseCounts for filter options (lightweight, no 10K response load)
      // This endpoint provides AC and interviewer lists without loading all responses
      try {
        const filterOptionsResponse = await surveyResponseAPI.getSurveyResponseCounts(surveyId, {
          includeFilterOptions: 'true'
        });
        
        if (filterOptionsResponse.success && filterOptionsResponse.data.filterOptions) {
          // Extract ACs and interviewers from filterOptions
          const filterOptions = filterOptionsResponse.data.filterOptions;
          
          // Build responses array from filterOptions for compatibility with existing code
          // This is much lighter than loading 10K actual responses
          const mockResponses = [];
          
          // Add ACs from filterOptions
          if (filterOptions.ac && filterOptions.ac.length > 0) {
            filterOptions.ac.forEach(ac => {
              mockResponses.push({
                selectedAC: ac,
                interviewer: null // Will be populated from interviewer filterOptions
              });
            });
          }
          
          // Add interviewers from filterOptions
          if (filterOptions.interviewer && filterOptions.interviewer.length > 0) {
            filterOptions.interviewer.forEach(int => {
              mockResponses.push({
                interviewer: {
                  _id: int._id || int.id,
                  firstName: int.firstName || int.name?.split(' ')[0] || '',
                  lastName: int.lastName || int.name?.split(' ').slice(1).join(' ') || '',
                  email: int.email || '',
                  phone: int.phone || '',
                  memberId: int.memberId || int.memberID || ''
                }
              });
            });
          }
        ...
        console.warn('Error fetching filter options (non-critical):', filterError);
        // Continue without filter options - stats will still work
      }
  ...
  useEffect(() => {
    if (surveyId && isMountedRef.current) {
      fetchSurveyData();
    }
    
    return () => {
      // Cleanup on surveyId change
      if (abortControllerRef.current) {
        abortControllerRef.current.abort();
        abortControllerRef.current = null;
      }
  ...
  // OPTIMIZED: Serialized filter key to detect changes efficiently (prevents memory leaks)
  // Like reports-v2 and responses-v2 pages - only triggers refetch when filters actually change
  const catiFiltersKey = useMemo(() => {
    return JSON.stringify({
      dateRange: catiFilters.dateRange,
      startDate: catiFilters.startDate,
      endDate: catiFilters.endDate,
      interviewerIds: Array.isArray(catiFilters.interviewerIds) 
        ? catiFilters.interviewerIds.sort() 
        : (catiFilters.interviewerIds || []),
      interviewerMode: catiFilters.interviewerMode || 'include',
      ac: catiFilters.ac || ''
    });
  }, [catiFilters.dateRange, catiFilters.startDate, catiFilters.endDate, catiFilters.interviewerIds, catiFilters.interviewerMode, catiFilters.ac]);
  ...
  }, [surveyId, survey, catiFiltersKey]); // Use filtersKey instead of individual filter dependencies
  ...
  // OPTIMIZED: Filter interviewers based on search term (memoized to prevent memory leaks)
  ...
  // OPTIMIZED: Filter ACs based on search term (memoized to prevent memory leaks)
  const filteredACs = useMemo(() => {
    if (!allACObjects || allACObjects.length === 0) return [];
    
    if (!catiAcSearchTerm || !catiAcSearchTerm.trim()) {
      return allACObjects;
    }
    ...
    return allACObjects.filter(acObj => {
      const name = (acObj.name || '').toLowerCase();
      const code = (acObj.code || '').toLowerCase();
      const numericCode = (acObj.numericCode || '').toLowerCase();
      
      // Match by name, code, or numeric code (supports exact and partial matches)
      const nameMatch = name.includes(searchLower);
      const codeMatch = code.includes(searchLower);
      ...
      return nameMatch || codeMatch || numericCodeMatch;
    });
  }, [allACObjects, catiAcSearchTerm]);
  ...
  // No loading screen - show content immediately (backend is optimized with aggregations)
          ...
          <div className="w-full px-4 sm:px-6 lg:px-8 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="relative" ref={catiAcDropdownRef}>
                  ...
                  {showCatiACDropdown && (() => {
                    // OPTIMIZED: Use memoized filteredACs instead of inline filtering (prevents memory leaks)
                    return filteredACs.length > 0 ? (
...
export default CallerPerformanceV2Page;
